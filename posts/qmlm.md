...
每个月份
根据2*2坐标系
x轴:内倾 外倾(I E)数值之后再设计
y轴:理想 现实(N S)
分为四个格子
然后每一月份都有对应四格子/或者三格子/或者两个格子的事件


9/10/11月份事件固定 初步了解仟虹和怜两个角色
默认
怜(E S)
仟虹(I N)


先从假如玩家选择怜的角度出发(目前 E S)


每个月有月考嘛 月考就按照加点和选择题概率来决定下一月份进到哪个剧情(())
月考成绩计算机制
对于怜: 

然后排列组合从12月份开始的话(12月份月考)


高二上学期
9月份下旬(怜搞完复赛后)
仟虹和怜刚见面
怜带仟虹参观学校可以参观学校等。(学校会有几个地点?)
地点还有家里等
(一些对话选项设计来计算 I/N E/S的值)
比如怜选择去思考仟虹说的话 N会增加
...
这三个月下来如果怜的倾向数值产生变化会有不同剧情线(不一定非要四个)
