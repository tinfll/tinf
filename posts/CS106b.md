...
貌似是cs50讲过的什么排序来着，反正就是喊人上去一点点摆那个场景记得倒挺清楚的，
于是立刻就去写了，发现不会写，
...
```
。我是墓碑僵尸。。。。。就是坑次坑次搞半天发现我最后禁用的测试案例没开。。（这个时候以为自己写的是错的就放弃去想其他办法了。实际上面for循环那个可以通过测试，虽然说也是O(n^2)的复杂度
然后改半天改成下面。虽然期间也有我的逆天逻辑链条。。
...
# ` RobinHood HashTable `
其实从表面机制实现起来感觉甚至比上一个线性哈希要容易，但是实际上已经被自己和claude逼疯，正好闲下来整理一下（实际代码是昨天水课快魔女化时弄完，逐渐失去对算法的兴趣（目移。      
...
bool RobinHoodHashTable::remove(const string& elem) {
    int dpos = -1;
    for (int i = 0; i < numSlots; i++) {
        if (elems[i].distance != EMPTY_SLOT && elems[i].value == elem) {
            dpos = i;
            break;
        }
    }
    ...
    while (true) {
        ...
        if (elems[n].distance == EMPTY_SLOT || elems[n].distance == 0)
            break;
...
## 11.05
下去去开了A8发现刚开始有些rpg味道，，简单指针+if-else吗？...还没去细化因为写着写着睡着了
